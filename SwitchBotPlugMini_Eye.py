...
class App:
    ...
    def scan(self):
        ...
        async def do():
            try:
                # return_adv=True で詳細なアドバタイジングデータを取得
                devices_dict = await asyncio.wait_for(BleakScanner.discover(timeout=5.0, return_adv=True), timeout=10.0)
                nms, found = [], []
                for addr, (device, adv_data) in devices_dict.items():
                    name = (device.name if device.name else "").lower()
                    uuids = [str(u).lower() for u in adv_data.service_uuids]
                    # Service DataのキーもUUID判定に含める
                    sdata_keys = [str(k).lower() for k in adv_data.service_data.keys()]
                    manuf = adv_data.manufacturer_data
                    
                    # より広い判定条件 (名称, Service UUID/Data:fd3d, メーカーID:2409)
                    is_sb = (any(kw in name for kw in ["plug", "mini", "wop", "switch", "woan"]) or
                             any("fd3d" in u for u in uuids + sdata_keys) or
                             2409 in manuf)
                    
                    if is_sb:
                        found.append(device.address)
                        base_name = device.name if device.name else "SwitchBot"
                        nms.append(f"{base_name} ({device.address})")
                
                self.root.after(0, lambda: self.update_dev_list(nms, found))
                self.up_s("完了", "black")
                self.play_sound()
            except asyncio.TimeoutError:
                self.up_s("エラー: タイムアウト", "red")
            except Exception as e:
                self.up_s(f"エラー: {type(e).__name__}", "red")
